  ...
  try {
    ...
    if (!userId || !businessName || !phone || !address) {
      return res.status(400).json({
        success: false,
        message:
          "Missing required fields: userId, businessName, phone, address",
      });
    }
...
const createMenuItem = async (
  ...
  try {
    ...
    const result = await providerService.createMenuItem(
      providerProfile.id,
      req.body,
    );
    res.status(201).json({
...
const updateMenuItem = async (
  ...
  try {
    ...
    const result = await providerService.updateMenuItem(
      providerProfile.id,
      mealId,
      req.body,
    );
...
const deleteMenuItem = async (
  ...
  try {
    ...
    const result = await providerService.deleteMenuItem(
      providerProfile.id,
      mealId,
    );
    res.status(200).json({
