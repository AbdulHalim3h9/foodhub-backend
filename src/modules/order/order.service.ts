...
const getProviderOrders = async (
  ...
  console.log(
    `🔍 [PROVIDER ORDERS] Fetching orders for providerId: ${providerId}`,
  );
...
const updateOrderStatus = async (orderId: string, status: string) => {
  ...
  if (!validStatuses.includes(status)) {
    throw new Error(
      "Invalid status! Must be one of: PENDING, CONFIRMED, PREPARING, READY, OUT_FOR_DELIVERY, DELIVERED, CANCELLED",
    );
  }
